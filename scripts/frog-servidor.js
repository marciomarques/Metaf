...
var tamanhoTotal = 0;
var thumbTop = '315px'; // distance you want you thumbnails to be from the top of their container
var loadingAni = 'images/loading.gif'; // image displayed when preloading images
var scalePercent = '100'; // percent thumbnails expand when fading into main image
...
Frog.prototype = {
	...
	initialize: function(){
		...
		tamanhoTotal = anchors.length;
/*
		var hid = document.createElement("div");
		hid.setAttribute("id", "galeria");
...
*/
		...
		for (var i=0; i<anchors.length; i++){
			...
			imageArray[i]['link'] = anchors[i].getAttribute('data-link');
			
			
/*
			var hud = document.createElement("a");
			hud.setAttribute("href", anchors[i].getAttribute('href'));
			hid.appendChild(hud);
*/
		}
/* 		divs.appendChild(hid); */
		...
		var mainImageA = document.createElement("a");
		mainImageA.setAttribute('rel', 'lightbox[gallery1]');
		mainImageA.setAttribute('id', 'FrogJSImageA');
		var mainImage = document.createElement("img");
		mainImage.setAttribute('id','FrogJSImage');
		mainImage.style.display = 'none';
		mainImageA.appendChild(mainImage);
		mainContainer.appendChild(mainImageA);
	...
	loadMainImage: function(imageNum, width){
		...
		function showMainImage(){
			$$('.current-count')[0].update(imageNum+1);
			$('FrogJSImageA').href = imageArray[imageNum]['full'];
			$('FrogJSImage').style.display = 'block';
			$('FrogJSImage').src = imageArray[imageNum]['full'];
			$('FrogJSMainContainer').style.width = '600px';//width+'px';
	...
	mainIn: function(imageNum, side){
		...
		Element.removeDimensions('FrogJS'+side+'Thumb2');
		Element.show('FrogJS'+side+'Thumb2');
		new Effect.Fade('FrogJS'+side+'Thumb2',{duration:0.1});
		new Effect.Scale('FrogJS'+side+'Thumb2', scalePercent, { duration: 0.1 });
	},
	...
	mainOut: function(imageNum, side){
		...
		if(imageArray[imageNum]){
			...
			imgPreloader.onload=function(){
				Element.removeDimensions('FrogJS'+side+'Thumb1');
				$('FrogJS'+side+'Thumb1').src = imageArray[imageNum]['thumb'];
				new Effect.Appear('FrogJS'+side+'Thumb1',{duration:0.1});
				new Effect.Scale('FrogJS'+side+'Thumb1', 100, { duration: 0.1, scaleFrom: scalePercent, afterFinish: function(){ addOnclick(); } });
				function addOnclick(){
					Element.setCursor('FrogJS'+side+'Thumb1','pointer');
					$('FrogJS'+side+'Thumb1').onclick = function(){
						myFrog.loadImage(imageNum, side, imgPreloader.width);
...
Event.observe(window, 'load', function(){
	$$(".total-count")[0].update(tamanhoTotal);
}, false);